testthat::test_that("Permanence of codes from claims data", {
  df_d <- data.frame(id = 1, index_date = Sys.Date(), date = Sys.Date() - 1, atc = "NOTaDRUG", npacks = 1, stringsAsFactors = F)
  ...
  for (c in seq_along(.multimorbidity_years())) {
    ...
    for (i in seq_along(.loc)) {
      ...
      df_h <- data.frame(id = 1, code = "NOTaCODE", index_date = index_date, date = index_date - 1, stringsAsFactors = F)
      hm <- max(1, .multimorbidity_howmany()[[c]]["claims"], na.rm = TRUE)
      if (is.null(.multimorbidity_years()[[c]])) {
        df_c <- data.frame(id = 1, code = rep(.loc[i], hm), index_date = index_date, date = index_date - seq(hm), stringsAsFactors = F)
      } else {
        df_c <- data.frame(id = 1, code = rep(.loc[i], hm), index_date = index_date, date = index_date - 400 * .multimorbidity_years()[[c]] * seq(hm), stringsAsFactors = F)
      }
