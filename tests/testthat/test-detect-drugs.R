testthat::test_that("Codes for drugs are detected properly: cpain", {
  ...
  for (c in .multimorbidity_drugs_atc()$cpain) {
    ...
    Dy1 <- data.frame(id = 1, index_date = Sys.Date(), date = Sys.Date() - seq(hm), atc = c, npacks = 1, stringsAsFactors = F)
    Dy2 <- data.frame(id = 1, index_date = Sys.Date(), date = Sys.Date() - 1, atc = c, npacks = hm, stringsAsFactors = F)
    Dn1 <- data.frame(id = 1, index_date = Sys.Date(), date = Sys.Date() - seq(hm), atc = "NOTaCODE", npacks = 1, stringsAsFactors = F)
    Dn2 <- data.frame(id = 1, index_date = Sys.Date(), date = Sys.Date() - seq(hm) - 366 * hy, atc = c, npacks = 1, stringsAsFactors = F)
...
testthat::test_that("Codes for drugs are detected properly: cpain for people with epilepsy", {
  Cn <- data.frame(id = 1, code = sample(x = .multimorbidity_codes()$epilepsy, size = 1), index_date = Sys.Date(), date = Sys.Date() - seq(.multimorbidity_howmany()$epilepsy["claims"]), stringsAsFactors = F)
  Hn <- data.frame(id = 1, code = "NOTaCODE", index_date = Sys.Date(), date = Sys.Date() - 1, stringsAsFactors = F)
  for (c in .multimorbidity_drugs_atc()$cpain_no_epilepsy) {
    hm <- .multimorbidity_drugs_howmany()$cpain_no_epilepsy
    hy <- .multimorbidity_drugs_years()$cpain_no_epilepsy
